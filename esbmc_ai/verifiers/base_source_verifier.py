...
from esbmc_ai.logging import log0, logv
...
class BaseSourceVerifier(BaseComponent):
    ...
    def _save_cached(self, properties: Any, result: Any) -> None:
        ...
        log0("Saving result to cache", post_label="Verifier")
        logv(f"Cache ID: {file_id}", post_label="Verifier")
    ...
    def _load_cached(self, properties: Any) -> Any:
        ...
        data_dump: bytes = pickle.dumps(obj=properties, protocol=-1)
        file_id: str = str(sha512(data_dump).hexdigest())
        logv(f"Searching cache ID: {file_id}", post_label="Verifier")
        ...
        if cache.exists() and filename.exists() and filename.is_file():
            with open(filename, "rb") as file:
                ...
                log0("Using cached result", post_label="Verifier")
                return data

        logv("Cache not found...", post_label="Verifier")
        return None
