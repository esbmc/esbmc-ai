...
def test_esbmc_get_counter_example(setup_get_data) -> None:
    ...
    assert ESBMCUtil._esbmc_get_counter_example(esbmc_output) == esbmc_output[ce_idx:]
...
def test_esbmc_get_violated_property(setup_get_data) -> None:
    ...
    assert (
        ESBMCUtil._esbmc_get_violated_property(esbmc_output)
...
def test_get_clang_err_line_index(setup_clang_parse_errors) -> None:
    ...
    line = ESBMCUtil._get_clang_err_line(data_esbmc_output["threading.c"])
    assert line == 26
